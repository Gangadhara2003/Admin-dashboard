...
export default function AdminDeliveriesPage() {
    ...
    return (
        <div>
            ...
            <div className="flex gap-1 bg-white/10 p-1 mb-5 overflow-x-auto">
                {tabs.map(t => (
                    <button key={t.key} onClick={() => setActiveTab(t.key)}
                        className={`px-3 py-2 text-xs font-medium transition-all whitespace-nowrap border-none cursor-pointer ${activeTab === t.key ? 'bg-white/5 text-accent ' : 'bg-transparent text-white/50 hover:text-white/80'
                            }`}>
                        {t.label}
                        <span className={`ml-1.5 px-1.5 py-0.5 rounded-full text-[10px] font-bold ${activeTab === t.key ? 'bg-accent/20 text-accent' : 'bg-gray-200 text-white/50'
                            }`}>{t.count}</span>
                    </button>
                ))}
            </div>
            ...
            {filtered.length === 0 ? (
                ...
                <div className="space-y-3">
                    {filtered.map((order: any) => {
                        ...
                        return (
                            <div key={order._id}
                                className={`bg-white/5 border p-4 transition-all hover: cursor-pointer ${computeSla(order) === 'breached' ? 'border-red-200 bg-red-500/10/30' : computeSla(order) === 'warning' ? 'border-yellow-500/20 bg-yellow-500/100/10/20' : 'border-white/10'
                                    }`}
            ...
            {/* Assign Delivery Boy Modal */}
            {assignModal && (
                <div className="fixed inset-0 bg-black flex items-center justify-center z-[1000]" onClick={() => setAssignModal(null)}>
            ...
            {/* Mark Failed Modal */}
            {failModal && (
                <div className="fixed inset-0 bg-black flex items-center justify-center z-[1000]" onClick={() => setFailModal(null)}>
                    <div className="w-full max-w-md bg-white/5 border-2 border-white/10 p-6" onClick={e => e.stopPropagation()}>
